...
class Panda(PyBulletRobot):
    ...
    def __init__(
        ...
        #self.neutral_joint_values = np.array([0.0, 0.41, 0.0, -2.1, 0, 2.26, 0.79, 0.1, 0.1])
        self.neutral_joint_values = np.array([0.4, 0.71, 0.0, -1.5, 0.0, 2.26, 0.79, 0.0, 0.00])
        #self.neutral_joint_values = np.array([ 0.30,  1.83,   0.02,  0.428,  -0.02,   1.39,1.078,  0.0, 0.0])
       # angles = self.inverse_kinematics(
          #  link=11, position=np.array([0.05, 0.2, 0.1]), orientation=np.array([1.0, 0.0, 0.0, 0.0]))
        #self.neutral_joint_values = np.array(angles)
        #print(angles)
        self.ee_link = 11
